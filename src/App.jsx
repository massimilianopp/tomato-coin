...
export default function App() {
  ...
  // --- CONFIGURATION ---
  const BUY_LINK = "https://pump.fun"; // Remplace par ton lien
  const MINT_ADDRESS = "ComingSoon_Or_Contract_Address"; // Remplace par l'adresse du jeton
  // ---------------------
      ...
      <div className="scanlines">
        ...
        <nav style={{ position: "fixed", top: 0, width: "100%", zIndex: 100, background: "rgba(4,4,10,0.9)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(255,32,32,0.2)" }}>
          <div className="container" style={{ height: 70, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div className="nav-links" style={{ display: "flex", gap: 35 }}>
              ...
              <a href="https://jup.ag/?buy=CdczQNrp2DZ9c89LSjCyRF6VmS4VtTCBkNSjXtpvmoon" target="_blank" className="cta-btn" style={{ padding: "8px 20px", fontSize: "0.65rem" }}>BUY</a>
            </div>
          </div>
        </nav>
        ...
        <div style={{ marginTop: 70, background: "rgba(255,32,32,0.08)", padding: "12px 0", borderBottom: "1px solid rgba(255,32,32,0.2)", overflow: "hidden" }}>
          <div className="ticker-track">
            {["$TOMATO", "SOLANA", "1B SUPPLY", "NO DEV ALLOCATION", "BURNED LP", "1B SUPPLY"].map((item, i) => (
              <span key={i} style={{ fontFamily: "Share Tech Mono", padding: "0 35px", color: i % 2 === 0 ? "var(--neon-red)" : "#888" }}>◆ {item}</span>
            ))}
          </div>
        </div>
          ...
          <div className="container hero-flex" style={{ display: "flex", alignItems: "center", gap: 60 }}>
            <div className="hero-text" style={{ flex: 1 }}>
              ...
              <h1 style={{ fontFamily: "Orbitron", fontSize: "clamp(45px, 9vw, 100px)", lineHeight: 1, fontWeight: 900 }}>
                ...
                <span style={{ color: "#fff", fontWeight: 400 }}>COIN</span>
              </h1>
              <p style={{ fontSize: "1.2rem", color: "#bbbbd0", margin: "25px 0 35px", maxWidth: 550 }}>
                The memecoin that <span style={{ color: "var(--neon-red)" }}>refreshes your portfolio</span>. 100% Community, 0% Dev greed.
              </p>
              
              <div className="stats-row" style={{ display: "flex", gap: 40, marginBottom: 40 }}>
                <div><div className="neon-text" style={{ fontSize: "1.8rem", fontWeight: 900 }}>1B</div><div style={{ fontSize: "0.6rem", color: "#666" }}>TOTAL SUPPLY</div></div>
                <div><div className="neon-text" style={{ fontSize: "1.8rem", fontWeight: 900 }}>0%</div><div style={{ fontSize: "0.6rem", color: "#666" }}>DEV ALLOC.</div></div>
              </div>

              <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
                <a href="https://jup.ag/?buy=CdczQNrp2DZ9c89LSjCyRF6VmS4VtTCBkNSjXtpvmoon" target="_blank" className="cta-btn">BUY $TOMATO</a>
                <a href="https://t.me/tomatocoinsolana" className="cta-btn cta-btn-green">TELEGRAM</a>
              </div>

              {/* MINT / CONTRACT BOX */}
              <div className="ca-box">
                <div style={{ fontSize: "0.6rem", marginBottom: 5 }}>CONTRACT ADDRESS (SOL)</div>
                {MINT_ADDRESS}
              </div>
            </div>
        ...
        {/* TOKENOMICS */}
        <section id="tokenomics" style={{ background: "var(--bg-panel)" }}>
          <div className="container">
            ...
            <div className="tokenomics-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 25 }}>
              {[
                { label: "Liquidity", value: "90%", desc: "Burned at launch." },
                { label: "Community", value: "100%", desc: "No dev allocation." },
                { label: "Marketing", value: "10%", desc: "From organic growth." },
                { label: "Tax", value: "0/0", desc: "No buy/sell tax." },
              ].map((t, i) => (
                <div key={i} className="corner-cut" style={{ background: "var(--bg-card)", padding: 35, border: "1px solid rgba(255,32,32,0.2)" }}>
                  ...
                  <div style={{ color: "var(--neon-green)", fontFamily: "Share Tech Mono", margin: "15px 0", fontSize: "0.8rem" }}>{t.label}</div>
                  <p style={{ fontSize: "0.9rem", color: "#888" }}>{t.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="roadmap" className="grid-bg">
          <div className="container">
            ...
            <div style={{ borderLeft: "2px solid var(--neon-red)", paddingLeft: 40 }}>
              {[
                { p: "PHASE 1", t: "GENESIS", s: "DONE", items: ["1B Supply Launch", "Community Build"] },
                { p: "PHASE 2", t: "CULTIVATION", s: "ACTIVE", items: ["Listing CG", "Tomato Merch"] },
                { p: "PHASE 3", t: "DOMINATION", s: "SOON", items: ["CEX Tier 1", "Global Sauce"] }
              ].map((phase, i) => (
                <div key={i} style={{ marginBottom: 50, position: "relative" }}>
                  ...
                  <div style={{ fontFamily: "Share Tech Mono", color: "var(--neon-red)", fontSize: "0.8rem" }}>{phase.p} · {phase.s}</div>
                  <h3 style={{ fontFamily: "Orbitron", fontSize: "1.5rem", margin: "10px 0" }}>{phase.t}</h3>
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    {phase.items.map(it => <span key={it} style={{ fontSize: "0.75rem", color: "#666", border: "1px solid #333", padding: "2px 8px" }}>{it}</span>)}
                  </div>
                </div>
              ))}
            </div>
        ...
        <footer style={{ padding: "60px 0", borderTop: "1px solid rgba(255,32,32,0.2)", background: "var(--bg-panel)", textAlign: "center" }}>
          <div className="container">
            <div style={{ fontFamily: "Orbitron", fontWeight: 900, color: "var(--neon-red)", letterSpacing: "4px" }}>TOMATOCOIN</div>
            <div style={{ fontFamily: "Share Tech Mono", color: "#555", fontSize: "0.7rem", marginTop: 15 }}>
              © 2026 · {time} · 1B SUPPLY · NO DEV ALLOCATION
            </div>
          </div>
        </footer>
      </div>
